...
results_path = "Cross_Site_Results_Cont/"
th = 15
thresh = 0.90 # alpha
            ...
            with open(file_path, 'r') as f:
                ...
                for key in data:
                    if data[key]['user_state'] == state:
                        ...
                        include_mean = np.mean([float(i) for i in data[key]['include']])
                        omit_mean = np.mean([float(i) for i in data[key]['omit']])
                        ...
                        expl_network_latencies[key][dt] = min([float(i) for i in data[key]['redirect_omit']])
                        expl_network_latencies_arr[key][dt] = [float(i) for i in data[key]['redirect_omit']]
    ...
    if not file.startswith('old_') and file.endswith('.json'):
        ...
        with open(file_path, 'r') as f:
            ...
            for key in data:
                ...
                attack_network_latency = min([float(i) for i in data[key]['redirect_measured_response_times']])
                ...
                filt_resp_times_omit_low_load = [max(x - expl_network_latencies[key][min_omit_dt[key]], 0.1) for x in expl_resp_times[key][min_omit_dt[key]]['omit']]  # X^0
                filt_resp_times_include_low_load = [max(x - expl_network_latencies[key][min_include_dt[key]], 0.1) for x in expl_resp_times[key][min_include_dt[key]]['include']]  # X^1
                filt_resp_times_expl = [max(x - expl_network_latencies[key][closest_date_string], 0.1) for x in expl_resp_times[key][closest_date_string]['omit']]  # X^R
                filt_resp_times_attack = [float(i) for i in data[key]['measured_response_times']]
                ...
                if EXECUTE_MATLAB:
                    ...
                    command = 'matlab -batch "cd(\'Model\'); getProbs_ps(' + resp_times_omit_low_load_str + ', ' + resp_times_include_low_load_str + ', ' + resp_times_expl_str + ', ' + resp_times_attack_str + ', ' + str(attack_network_latency) + ', ' + str(th) + ');"'
                    print(command)
                ...
                lastl = []
                os.makedirs(f'{results_path}/{key}', exist_ok=True)
                if EXECUTE_MATLAB:
                ...
                for line in liness:
                    ...
                    with open(f'{results_path}/{key}/{key}_results_{dt}.csv', 'a', newline='') as csv_file:
                        writer = csv.writer(csv_file)
                        if i == 1:
                            writer.writerow(['num_attack_obs', 'pr_omit', 'pr_include', 'expected_result', 'last_attack_resp_added', 'th', 'l', 'mu', 'rho'])
                        writer.writerow([i, output_arr[1], output_arr[3], data[key]['expected_result'], output_arr[11], th, output_arr[5], output_arr[7], output_arr[9]])
...
for key in total_all:
    ...
    df_cm.to_csv(results_path + "/" + key + "/" + key + "_confusion_matrix.csv", index=True)
    ...
    df_cm.to_csv(results_path + "/" + key + "/" + key + "_confusion_matrix_bt.csv", index=True)
    with open(results_path + "/" + key + "/" + key + "_confusion_matrices_unknown.txt", "a") as file:
    ...
    with open(results_path + "/" + key + "/" + key + "_confusion_matrices_rates.txt", "a") as file:
...
with open(results_path + "/overall_confusion_matrices_rates.txt", "a") as file:
