...
results_path = "Cross_Site_Number_of_Requests_Results/"
th = 15
nbins = 4
thresh = 0.90 # alpha
    ...
    if not file.startswith('old_') and file.endswith('.json'):
        ...
        with open(file_path, 'r') as f:
            ...
            for key in data:
                ...
                lastl = []
                os.makedirs(f'Cross_Site_Number_of_Requests_Results/{key}', exist_ok=True)
                if EXECUTE_MATLAB:
                ...
                for line in liness:
                    ...
                    with open(f'Cross_Site_Number_of_Requests_Results/{key}/{key}_results_{dt}.csv', 'a', newline='') as csv_file:
                ...
                baking_timer(expl_resp_times_nl[key][closest_date_string_nl]['include'], expl_resp_times_nl[key][closest_date_string_nl]['omit'], expl_resp_times[key][closest_date_string]['include'],
                            expl_resp_times[key][closest_date_string]['omit'], resp_times_attack[:attack_length], resp_times_attack_omit[:attack_length], key, data[key]['expected_result'], tp_count_bt, fp_count_bt, tn_count_bt, fn_count_bt, unk_count_bt)
...
for key in total_all:
    ...
    df_cm.to_csv("Cross_Site_Number_of_Requests_Results/" + key + "/" + key + "_confusion_matrix.csv", index=True)
    ...
    df_cm.to_csv("Cross_Site_Number_of_Requests_Results/" + key + "/" + key + "_confusion_matrix_bt.csv", index=True)
    with open("Cross_Site_Number_of_Requests_Results/" + key + "/" + key + "_confusion_matrices_unknown.txt", "a") as file:
    ...
    with open("Cross_Site_Number_of_Requests_Results/" + key + "/" + key + "_confusion_matrices_rates.txt", "a") as file:
...
with open("Cross_Site_Number_of_Requests_Results/overall_confusion_matrices_rates.txt", "a") as file:
